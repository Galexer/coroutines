                ...
                val posts = getPosts(client)
                    .map { post ->
                        async {
                            ...
                            val commAuthors = comments.map{com -> getPostsAuthors(client, com.authorId) }
                            PostWithComments(post, getPostsAuthors(client, post.authorId), comments, commAuthors)
                        }
                    }.awaitAll()
                println(posts)
            } catch (e: Exception) {
...
suspend fun getPostsAuthors(client: OkHttpClient, authorId: Long): Author =
